...
class BuildAssetsCommand(BaseBuildCommand):
    ...
    def _get_package_version(self):
        ...
        try:
            ...
            log.info(f"pulled version information from 'cosmos' module. {cosmos.__file__}")
            version = self.distribution.get_version()
            build = cosmos.__build__
        finally:
            sys.path.pop(0)
    ...
    def cosmos_static_dist_path(self):
        return os.path.abspath(os.path.join(self.build_lib, "src/static/cosmos/dist"))
...
VERSION = "0.1.0.dev0"
IS_LIGHT_BUILD = os.environ.get("COSMOS_LIGHT_BUILD") == "1"
...
class CosmosSDistCommand(SDistCommand):
...
class CosmosBuildCommand(BuildCommand):
    def run(self):
        if not IS_LIGHT_BUILD:
...
class CosmosDevelopCommand(DevelopCommand):
    def run(self):
...
cmdclass = {
    "sdist": CosmosSDistCommand,
    "develop": CosmosDevelopCommand,
    "build": CosmosBuildCommand,
...
setup(
    ...
    entry_points={
        "console_scripts": ["cosmos = cosmos.main:entrypoint"]
    },
)
